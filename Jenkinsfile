pipeline {
    agent any
    environment {
        registry = "seifallahesprit/kaddem"
    ...
    stages {
        ...
        stage('Building docker  image') {
            steps {
                script {
                    sh "docker build ./ -t seifallahesprit/kaddem:dev${new_commitShort}"
                }
            }
        }
            ...
            steps{
                script {
                    docker.withRegistry('', registryCredential) {
                        sh "docker push seifallahesprit/kaddem:dev${new_commitShort}"
                    }
                }
            }
        }
        ...
        stage('cleaning image'){
            steps{
                script {
                    sh "docker rmi seifallahesprit/kaddem:dev${new_commitShort}"
                }
            }
        }
